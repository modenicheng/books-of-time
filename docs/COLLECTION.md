...
### Cohort State And Policy Foundation

C2 已建立下一代快照 cohort 的持久状态和纯策略，但没有改变当前请求所有权：
...
- task/coverage 已预留可空 cohort/component 逻辑 ID；旧任务仍可保持 NULL。
- UTC 对齐、上海活跃窗口、6/12/18/24 小时 checkpoint、S/A/B/C OR 评级、两次降级确认和 active/dormant/archived 规则已经是可测试纯函数。

配置模板仍是 `snapshot_cohorts.enabled: false`。C2 repository 只保存 policy 和视频状态，不创建 cohort、component 或 task。当前 `video_snapshot_sweep` 和 collector 递归调度仍是常规视频指标的唯一 owner；C3 才加入 shadow planning，后续验收通过后再迁移所有权。因此本节描述的是可用数据契约，不表示评论或 cohort 已开始自动周期调度。
...
`snapshot_cohorts.enabled` 在 C2 不注册 scheduled job，也不改变以上 owner。五张 cohort/policy 表可以存在且保持空；这不是运行故障。
