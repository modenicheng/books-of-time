...
因此 UID discovery 是“持续发现账号最新投稿”，不是账号全历史导入。
...
详细轮询窗口当前固定为 `10:00 <= Asia/Shanghai 本地小时 < 22:00`。模板中的 `scheduler.core_start_hour`、`core_stop_hour` 和 `timezone` 尚未接入此策略。每天 22:00 后，terminal job 为当日已发现且仍可用的视频生成一次幂等终结快照。
...
- 已入队 task 派生的 latest follow-up、watchlist reply 和 media download。
...
这些能力已经可用，但需要 operator、外部定时器或后续项目调度策略显式入队/执行。不要仅因服务在运行就假设评论快照会自动持续刷新。
