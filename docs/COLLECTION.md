...
### Persistent Cohort Planner, C5 Hot/Latest Plans And Shadow Mode

C5 已在 C3 持久 planner 之上加入分层热门扫描和可恢复 latest scan，但仍没有改变当前请求所有权：
...
- `snapshot_cohorts` / `snapshot_cohort_components` 保存计划时间点、组件状态和固定页范围；`collection_schedule_gaps` 保存未物化或未执行的时间范围。
- 原子物化在一个 caller-owned 事务中锁定单视频 state，创建唯一 cohort、缺失 component 和首任务；shadow 调用同一决策路径，但禁止创建 task。
- 底层 live 物化会为 `hot_core` / `hot_deep` 创建持久 scan，并为 latest 创建或加入单 BVID active scan；task、coverage、raw page 与 comment observation 都可直接回溯 scan，旧独立任务仍保持 NULL。
- UTC 对齐、上海活跃窗口、6/12/18/24 小时 checkpoint、S/A/B/C OR 评级、两次降级确认和 active/dormant/archived 规则已经是可测试纯函数。

配置模板仍是 `snapshot_cohorts.enabled: false`。设置为 `true` 且保持 `rollout_mode: shadow` 后，scheduler 每 30 秒评估 known videos：首次采纳固定 pubdate anchor，计算 tier/lifecycle，物化到期 routine/checkpoint，压缩 stale routine 为 gap，并把超过 60 分钟的历史 checkpoint 合并成一个当前 recovery 计划。所有父 cohort 使用 `shadow_planned`，`extra.shadow_target_status` 保存若 live 时应得到的状态；组件会保存 C5 热门范围和 latest current-head 合同，但 shadow 不创建 scan run 或 task。
...
当前 `video_snapshot_sweep`、daily terminal job 和 collector 递归调度仍是 live 视频指标 owner；外部 timer 仍负责周期 hot/latest 入队。C5 的 service 构建器拒绝 `rollout_mode: live`，所有权迁移在 C7 完成。因此 shadow 表中的热门/latest 计划不能被解释为评论已经自动抓取。

底层 live 执行语义已经固定供 C7 复用：worker 领取首任务时把 component/cohort 标为 running，并记录 task 生命周期的 `started_at`；`skew_seconds` 只在首个真实 HTTP attempt 落库时按 `request_started_at - scheduled_for` 写入，后续分页和重试不覆盖。没有发出 HTTP 的 task 保持 NULL skew。hot component 从单 scan 累计计数；多个 latest component 可共享一个 scan，按各自 capture window 独立完成并重算所有父 cohort。可重试失败保持 owner/消费者 active，最终失败才以 CAS 清 owner 并扇出 failed/corrupted。
...
### C4 `hot_core` / `hot_deep` Scans

`cohort-default-v2` 把 routine 热门页和 checkpoint/首次 active 采纳总页数分开：

| Tier | routine `hot_core` | checkpoint/首次采纳 `hot_core` | `hot_deep` 补页 | 总页数 |
| --- | --- | --- | --- | --- |
| S | 1-3 | 1-3 | 4-20 | 20 |
| A | 1-2 | 1-2 | 3-10 | 10 |
| B | 1 | 1 | 2-3 | 3 |
...
普通 routine 只执行表中第一列，不创建 deep。首次把 active 视频纳入 cohort 时按 checkpoint 深度记录 t0；6/12/18/24 小时 checkpoint 也使用总页数。dormant routine 只保留热门第 1 页，archived cohort 不含热门组件。`hot_deep` 是总目标扣除 core 后的余量，不会再次从第 1 页采集。

每个组件拥有一个 `comment_scan_runs` 行，`next_page_number` 是唯一恢复进度。一个 task slice 最多成功采集 10 页，并在运行达到 55 秒后让出；只有已经成功至少一页后才检查让出，因此不会产生零进度续片。让出时 scan 标为 `paused`，下一 slice 与进度更新在同一个 worker 事务提交。进程重启或当前 task 请求失败后继续读取 `next_page_number`，已经成功的页不会重放；同一页的失败重试会增加 `pages_requested`，成功页只增加一次 `pages_succeeded`。

slice 使用 `(comment_scan_run_id, phase, slice_no)` 生成 `scan_slice_key`。数据库在所有 task 状态上唯一约束该 key，所以旧 slice 即使已经 succeeded 或 failed，重复 planner、重启或竞争 worker 也不能创建第二条。普通 `idempotency_key` 仍只约束活动 task，不替代这个全状态身份。

scan `status` 表示生命周期，`outcome` 表示为什么停在该状态：

| `status` | 常见 `outcome` | 含义 |
| --- | --- | --- |
| `planned` / `running` | NULL | 尚未开始或 slice 正在执行 |
| `paused` | `time_slice_yield` | 非终态，已有下一编号 slice 或即将在同事务创建 |
| `complete` | `target_reached` | 已成功采到配置范围最后一页 |
| `complete` | `server_end` | 平台明确 `is_end=true`，或成功返回空评论页；该末页仍计入 requested/succeeded |
| `partial` | 具体截断原因 | 终态但未满足完整目标 |
| `failed` | `retry_exhausted` 等 | transport/collector 失败耗尽 task 重试 |
| `corrupted` | `retry_exhausted` 等 | parse 或状态证据损坏，不能声明完整 |

每页仍有自己的 HTTP 捕获时间，不把 20 页描述成平台事务快照。`raw_page_observations.scan_run_id`、`comment_observations.scan_run_id` 和 `collection_coverage_stats.comment_scan_run_id` 提供直接证据链；通用 `raw_payloads` 经同时保存 `raw_payload_id` 与 `scan_run_id` 的 raw page 归属到 scan，task 与 cohort component 也保存同一个 scan ID。页面 raw、结构化 observation、slice coverage 和 scan 累计计数应联合验收，不能只看 task 是 succeeded。

## 7. Latest Comments Baseline And Frontier

最新评论使用平台 cursor，不使用固定页号作为唯一进度。当前有两条兼容路径：

- 手工 legacy task 的 `comment_scan_run_id IS NULL`，继续使用原有 CLI collector。
- cohort task 带 scan ID，使用 C5 状态机；同一 BVID 的 tail、head sweep、incremental
  在任意时刻最多一个 active scan，多个 cohort component 可以加入同一个 scan。

scan-backed slice 最长 55 秒，高频 routine 会按配置文档中的公式缩短到 10-55 秒。
每个 slice 使用稳定的 `(scan_id, mode, slice_no)` 身份。让出前通过 CAS 持久化 cursor、
retry progress 和 frontier version，进程重启或 task retry 从该状态继续。
...
第一张成功页面记录最多五个有序 `start_anchor_set` 项；primary 项同时写入兼容字段
`start_frontier_rpid`。它们只是 baseline 开始时看到的头部锚点，还不是正式增量
frontier。每个成功页面都会把 scan ID 写入 raw page、comment observation 和 coverage。

### First Baseline: Head Sweep

tail 到达服务端末尾后，collector 在同一事务中完成 parent tail，创建确定性的 child
`baseline_head_sweep` 及 slice-0 task，把 `active_scan_run_id` 移交给 child，并重绑所有
仍有效的消费者。head sweep 从当前 head 回扫，直到遇到任一保留起点：

```text
baseline_tail_complete
  -> baseline_head_sweep paused       time slice; numbered follow-up queued
  -> baseline_head_sweep complete     any retained start anchor reached
  -> baseline_head_sweep corrupted    no anchor at server end, retry exhaustion, or cursor loop
```

这一步补采 tail scan 期间新出现的评论。成功的第一张 head 页面记录
`head_captured_at`，即使逻辑 sweep 后续因深页让出，它仍可作为 cohort current-head
证据。若初始 tail 明确为空，则直接建立空 frontier，不发出冗余 child 请求。

### Incremental Scan

baseline complete 后，每次从 head 扫描：

- 任一页面包含最多五个旧锚点之一：`complete/frontier_reached`，用第一张成功头页的
  有序锚点替换正式 frontier。
- 旧 frontier 明确为空且到达服务端末尾：`complete/frontier_reached`，正式 frontier
  仍为空。
- 时间片耗尽：`paused/time_slice_yield`，保留 cursor、retry progress 和正式 frontier，
  自动创建 numbered follow-up。
- 请求重试耗尽或 cursor loop：`corrupted`，正式 frontier 保持不变。
- 非空旧锚点全部未出现而到达服务端末尾：`partial/frontier_missing`，在 scan/frontier
  证据中记录所有缺失 RPID，为兼容 primary 写 `missing_after_seen`，并把本轮头部候选
  提升为下一 frontier。

匹配任一锚点可避免“primary 单条评论被删”直接破坏整个 frontier。cursor 分页期间仍会
受新增、折叠、删除和排序变化影响；系统保留每页 raw 与 observation，不声称多页是
平台事务快照。

### Retry And Corruption

同一 cursor 默认最多尝试 3 次，退避序列为 1、3、5 秒。重试状态保存在
`frontier_states.extra`，跨 follow-up 延续。尝试耗尽标为 corrupted；corrupted 不等于
complete，也不会自动从零重建 baseline。一次失败后若同 slice 重试成功，请求结果会
连同最新 CAS frontier/progress 返回持久化路径，不依赖 ORM 对象是否原地刷新。

### Cohort Consumers And Deadlines

latest component 加入 active scan 后为 `joined_active_task`。历史 tail 页面不能满足
current-head 合同；只有 head sweep 或 incremental 的第一张成功头页，其
`head_captured_at` 落在组件半开窗口 `[scheduled_for, deadline)` 内时，组件才 complete。
窗口起点晚于该 capture 的消费者保持 joined，并在当前 scan 关闭后重绑下一 scan。

deadline 到达仍未取得合格头页时，组件以 `partial` 结束：

```text
baseline_tail_in_progress
current_head_not_captured
```

执行仓储会复制逻辑 scan 的累计计数（包括 tail parent 与 head child），同步所有消费者，
并对每个受影响父 cohort 加行锁重算。可重试失败保持 scan owner 与消费者 active；重试
耗尽时，transport/collector 失败为 `failed/retry_exhausted`，parse 失败为
`corrupted/retry_exhausted`，且只用 CAS 清除匹配的 frontier owner。陈旧 worker 不能
清除更高版本 owner。

30 秒 planner repair 是有界且幂等的：修复 planned/paused scan 缺失的 numbered task，
保留原 priority/budget/retry 参数，把未过期消费者加入当前 active scan，或从 durable
frontier 创建下一 scan。`shadow_planned` cohort 永远不会进入该执行路径。

### Consistency Boundary

cursor 分页也不是数据库快照。系统通过 rpid 实体去重、raw 页面留证、tail + head
sweep、多锚点和明确 outcome 降低歧义，但不能证明采集期间平台集合完全静止。
...
scan-backed baseline 必须同时检查 parent tail 和 child head。只有 child
`start_anchor_reached`、`corrupted=false` 且 current-head component 没有未解决 deadline
时，才可视为完成。`baseline_tail_complete`、`paused`、`failed`、`corrupted` 都不是完整
baseline。legacy task 需通过 NULL scan ID 区分，并按其兼容 coverage 合同验收。
...
- 配置启用时，每 30 秒执行 snapshot cohort shadow planning，写入 C5 热门页范围和 latest current-head 合同，但不创建 scan/task。
...
由于 C7 尚未迁移 live owner，`service run` 当前不会自行周期入队：

- 每个视频的热门评论快照。
- scan-backed cohort latest baseline/增量；其底层 tail -> child head 自动衔接尚未由服务激活。
- baseline complete 后的周期 latest 增量。
- media similarity analysis。
- 事件分析和报告。

这些能力的底层实现已经可用，但仍需要 operator、外部定时器或后续 C7 live 迁移显式
入队/执行。legacy 评论 task 一旦入队便可在任意时间被 worker 领取；“全天运行”和
...
- scan-backed latest baseline 的 `frontier_states.extra.baseline_status` 最终是
  `baseline_complete`，同时 parent tail 与 child head 都必须按各自 coverage 合同验收；
  child 的 `start_anchor_reached` 是 scan 终态，单独的 `tail_reached` 仍只是中间证据，
  不是完整 baseline。
