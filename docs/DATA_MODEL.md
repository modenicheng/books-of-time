...
- 可空 `scan_run_id`：C4 编号热门或 C5 scan-backed latest 页面的直接归属；旧独立页面为 NULL
- `extra`：next offset、is_end、folder 证据、discovery 来源等

它是 raw bytes 和结构化 observation 之间的页面级证据节点。`raw_payloads` 保持请求类型通用，不直接增加 scan 外键；C4/C5 通过同一行的 `raw_payload_id` + `scan_run_id` 把原始响应归属到逻辑扫描。
...
- raw payload/page 引用。
- 可空 `scan_run_id`，与产生该 observation 的热门逻辑扫描直接关联。
...
`UNIQUE(cohort_id, component_kind)`。字段包含 required、状态、scheduled/deadline/start/finish、skew、计划/请求/成功页数、item/raw 计数、可空 `comment_scan_run_id`、failure reason 和 extra。C4 的 `hot_core` / `hot_deep` component 在 `extra` 固定保存 start/end/target page 和 slice 限制；C5 latest component 保存 `max_scan_seconds`、`current_head_required` 和 task repair 元数据。多个 latest component 可以指向同一 scan，`joined_active_task` 表示共享逻辑任务，不表示已经取得 current-head 证据；shadow component 保持 scan ID 为 NULL。
...
C3-C5 物化器在 `video_collection_states` 行锁内创建 cohort/component/首任务，并由唯一约束处理竞争。recovery 可以给同一稳定 key 补入新的缺失 component；普通 routine/checkpoint 在首次事务完成后不能改变 component 集或计划页数。shadow component 保存计划状态和固定热门页范围，但没有 scan run 或 task。
...
持久任务队列：kind、target、idempotency key、priority、budget cost、status、payload、not-before、lease、retry count/max retries、可空 `snapshot_cohort_id` / `snapshot_cohort_component_id`、`comment_scan_run_id`、`scan_slice_no`、`scan_slice_key` 和时间戳。

活动 idempotency key 使用 PostgreSQL/SQLite 条件唯一索引，只约束 pending/running/backoff。

Cohort 首任务还按 `snapshot_cohort_component_id` 查询所有 task 状态，因此普通组件首任务即使已经 succeeded/failed，重复 planner 也不会创建替代行。非 NULL `scan_slice_key` 由普通唯一约束覆盖全部 task 状态；同一 scan/mode/slice 即使旧行已 succeeded/failed 也不能重建，缺失行才由 planner 按稳定 key 修复。C4 hot 和 C5 latest 都使用 `<scan_id>:<mode>:<slice_no>`；latest follow-up 同时保留 `frontier_version`，worker 会拒绝陈旧 owner。

### `comment_scan_runs`

评论逻辑扫描的持久、可恢复状态。`scan_key` 唯一；C4 为每个 live `hot_core` / `hot_deep` component 建一行，C5 latest task 共享一行，shadow 不建行。主要字段包括：

- BVID/OID、cohort 与可空 parent scan 关联、mode、policy version。latest baseline tail 的 child 是确定性的 `baseline_head_sweep`，不能和另一个 active latest 重叠。
- `target_pages`、`next_page_number`，以及 `extra.start_page` / `extra.end_page` 的不可变热门页范围。
- requested/succeeded page、item、raw 和 slice 累计计数；父子 latest scan 的 component 视图使用累计值。
- latest 的 `start_anchor_set` / `result_anchor_set` 最多五个有序 `{rpid, platform_created_at}`，primary RPID 保留在兼容字段；`start_cursor` / `result_cursor` 保存逻辑边界。
- latest `extra.head_captured_at` 是第一张成功头页的证据时间；`missing_anchor_rpids` 和 retry/cursor progress 保留失败上下文。
- status、outcome、truncated、最后错误和开始/完成时间。

status 严格为：

```text
planned, running, paused, complete, partial, failed, corrupted
```

status 与 outcome 正交。hot 的 `paused/time_slice_yield` 可按 page cursor 继续；latest 的 `paused/time_slice_yield` 按 frontier cursor 和 numbered slice 继续。latest baseline tail 到末尾为 `complete/tail_reached` 并自动创建 head child；head 命中任一起始锚点为 `complete/start_anchor_reached`；增量命中旧锚点或空 frontier 末尾为 `complete/frontier_reached`；非空锚点全部缺失为 `partial/frontier_missing`；retry exhaustion/cursor loop 为 `corrupted`。transport/collector task 重试耗尽由 worker 记录 `failed/retry_exhausted`，parse 失败记录 `corrupted/retry_exhausted`。`pages_requested` 计请求尝试，`pages_succeeded` 计成功页面；成功后才把 page/cursor progress 前移，所以重启和 task retry 不会重放已经成功的页。
...
- 可空 `comment_scan_run_id`，用于把每个 slice coverage 连接到同一逻辑扫描。

该表用于 `coverage`、event coverage、service status failure window 和 operational alerts。

Worker 开始 live cohort task 时，把 component/cohort 改为 running，并记录 task 生命周期的 `started_at`；joined component 被实际 owner task 领取时允许从 `joined_active_task` 转为 running。`skew_seconds` 使用首个关联 HTTP attempt 的 `request_started_at - scheduled_for`，后续请求不覆盖；没有真实 HTTP attempt 时保持 NULL。普通 task 按 coverage 聚合；scan-backed task 则从 `comment_scan_runs` 的累计计数和终态同步所有共享消费者，避免多 slice 或中途失败漏掉已经提交的成功页。只有合格 `head_captured_at` 才完成 current-head component；paused/running scan 使未完成消费者保持 joined/active，耗尽重试才映射 failed/corrupted。父 cohort 在同一事务中重新聚合，complete 时更新 `video_collection_states.last_completed_cohort_at`。
...
- 最多五个有序正式 frontier anchors、当前 cursor。
- `active_scan_run_id`：当前物理 owner；`version`：每次 mutable frontier update 递增的 CAS 版本。
- last scan 状态、页数和 truncated。
- `extra` 中的 baseline status、head progress、失败 cursor/attempts、missing anchors 等。

这是可恢复状态机，不是历史日志；每轮历史在 coverage/raw/page observations。对
`frontier_states` 的 mutable mutation（cursor、retry progress、active owner 和正式
frontier）都必须带 expected version；版本冲突代表陈旧 worker，不能覆盖官方 frontier。
普通 scan counter、component 同步和 cohort 聚合使用各自事务中的行锁，不宣称它们都由
frontier CAS 保护。只有 head/incremental terminal reached 或 frontier_missing 才更新正式
anchors；paused/corrupted 只保存恢复证据并保持旧 frontier。PostgreSQL 还用 latest mode
与 active status 组成的 partial unique index 保证一个 BVID 只有一个 active latest scan；
SQLite 只用于单进程测试。
