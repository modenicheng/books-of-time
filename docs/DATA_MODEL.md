...
当前 schema 对 event archive、`comment_analysis_flags`、media 核心关系，以及
`known_video_sources` 和 `http_request_attempts` 的关键归属建立数据库 foreign
key。大量高写入证据字段，例如 `comment_observations.raw_payload_id`，仍是应用维护
的逻辑引用，而不是 PostgreSQL 强制 foreign key。
