...
```mermaid
flowchart LR
    RP["raw_payloads"] --> PG["raw_page_observations"]
...
当前 schema 只对 event archive 和 `comment_analysis_flags` 的核心关系建立数据库 foreign key/cascade。大量高写入证据字段，例如 `comment_observations.raw_payload_id`、`comment_observation_media.media_asset_id`，是应用维护的逻辑引用，而不是 PostgreSQL 强制 foreign key。
...
后续正文变化不会覆盖 `first_content`；`updated_at` 只表示实体再次被看到。
...
discovery 见过的视频集合：BVID、来源 MID、pubdate、first seen。它驱动自动视频快照 sweep。
...
持久任务队列：kind、target、idempotency key、priority、budget cost、status、payload、not-before、lease、retry count/max retries 和时间戳。
...
| Hash | 计算对象 | 是否唯一约束 | 目的 |
| --- | --- | --- | --- |
| raw `url_hash` | 请求 URL 字符串 | 否 | 不保存完整 URL 时识别请求 |
| raw `params_hash` | 排序后的请求参数 JSON | 否 | 参数指纹 |
...
```sql
SELECT id, bvid, rpid, captured_at, sort_mode, position,
