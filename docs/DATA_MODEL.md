...
- 可空 `scan_run_id`：C4 编号热门扫描的直接归属；旧独立页面为 NULL
- `extra`：next offset、is_end、folder 证据、discovery 来源等

它是 raw bytes 和结构化 observation 之间的页面级证据节点。`raw_payloads` 保持请求类型通用，不直接增加 scan 外键；C4 通过同一行的 `raw_payload_id` + `scan_run_id` 把原始响应归属到逻辑扫描。
...
`UNIQUE(cohort_id, component_kind)`。字段包含 required、状态、scheduled/deadline/start/finish、skew、计划/请求/成功页数、item/raw 计数、可空 `comment_scan_run_id`、failure reason 和 extra。C4 的 `hot_core` / `hot_deep` component 在 `extra` 固定保存 start/end/target page 和 slice 限制；shadow component 保持 scan ID 为 NULL。
...
C3/C4 物化器在 `video_collection_states` 行锁内创建 cohort/component/首任务，并由唯一约束处理竞争。recovery 可以给同一稳定 key 补入新的缺失 component；普通 routine/checkpoint 在首次事务完成后不能改变 component 集或计划页数。shadow component 保存计划状态和固定热门页范围，但没有 scan run 或 task。
...
Cohort 首任务还按 `snapshot_cohort_component_id` 查询所有 task 状态，因此首任务即使已经 succeeded/failed，重复 planner 也不会创建替代行。非 NULL `scan_slice_key` 由普通唯一约束覆盖全部 task 状态；同一 scan/phase/slice 即使旧行已 succeeded/failed 也不能重建。C4 热门续片使用 `<scan_id>:<mode>:<slice_no>`，C5 可复用同一约束。

### `comment_scan_runs`

评论逻辑扫描的持久、可恢复状态。`scan_key` 唯一；C4 为每个 live `hot_core` / `hot_deep` component 建一行，shadow 不建行。主要字段包括：

- BVID/OID、cohort 与可空 parent scan 关联、mode、policy version。
- `target_pages`、`next_page_number`，以及 `extra.start_page` / `extra.end_page` 的不可变热门页范围。
- requested/succeeded page、item、raw 和 slice 累计计数。
- frontier/anchor/cursor 预留字段，供 C5 latest 扫描复用。
...
status 与 outcome 正交。`paused/time_slice_yield` 是可继续状态；`complete/target_reached` 表示达到配置页范围；`complete/server_end` 表示明确末页或成功空页提前结束。transport 重试耗尽为 failed，parse 证据损坏为 corrupted。`pages_requested` 计请求尝试，`pages_succeeded` 计成功页面；成功后才把 `next_page_number` 前移，所以重启和 task 重试不会重放已经成功的页。
...
Worker 开始 live cohort task 时，把 component/cohort 改为 running，并记录 task 生命周期的 `started_at`。`skew_seconds` 使用首个关联 HTTP attempt 的 `request_started_at - scheduled_for`，后续请求不覆盖；没有真实 HTTP attempt 时保持 NULL。普通 task 按 coverage 聚合；scan-backed task 则从 `comment_scan_runs` 的累计计数和终态同步 component，避免多 slice 或中途失败漏掉已经提交的成功页。paused/running scan 使 component/cohort 保持 running，耗尽重试才映射 failed/corrupted。父 cohort 在同一事务中重新聚合，complete 时更新 `video_collection_states.last_completed_cohort_at`。
