...
C3 已把持久 planner 接入 scheduler，但当前服务只允许 `shadow`。模板保持 `enabled: false`，便于升级后先完成 migration、doctor 和容量检查；改为 `true` 后会注册一个 30 秒 planner job，写入 policy、video state、cohort、component 和 schedule gap 证据，但不会创建 `collection_tasks`，也不会发起 Bilibili 请求。

`rollout_mode: live` 在 C3 会让 `service run` 启动失败并给出 C7 所有权迁移提示。这是刻意的安全边界：现有 `video_snapshot_sweep`、日终任务和 collector 递归调度仍在运行，提前启用 live 会造成重复采集。底层 live 物化和 worker 关联已经有测试覆盖，只供 C7 迁移复用，不是当前运维开关。

| 键 | 含义 |
| --- | --- |
| `enabled` | 是否注册持久 planner job；C3 启用后仍只运行 shadow |
| `policy_version` | 不可变策略内容的全局版本 ID；默认 `cohort-default-v1` |
| `rollout_mode` | C3 仅允许 `shadow`；`live` 留给 C7 所有权迁移 |
...
`policy_version` 与 policy 内容一一对应。planner 首次看到该版本时会把标准化策略 JSON 写入 `collection_policy_versions` 并激活；同一版本再次启动时必须得到完全相同的内容。修改 checkpoint、阈值、窗口、生命周期、间隔或 planner 周期时，应同时改成新的版本名，例如 `cohort-default-v2`。复用旧版本名但改变内容会明确失败，避免历史 cohort 的策略依据被静默改写。
...
shadow 可在 Docker、Linux 原生和 Windows 原生服务中使用，三者连接同一个外部 PostgreSQL 时共享 scheduled-job lease 和 planner 状态。SQLite 只支持单进程开发，不用于验证多 scheduler 并发。
