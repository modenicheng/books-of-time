...
Books of Time 是长期运行的应用服务。PostgreSQL 由宿主机或局域网现有实例提供，应用不会启动数据库容器，也不会在正常启动时自动迁移 schema。raw payload 与 media asset 始终写入本地持久目录。

## Deployment Contract

所有部署共享以下顺序：

1. 备份 PostgreSQL、raw 和 media。
...
默认绑定 `${BOT_DATA_DIR}/raw` 和 `${BOT_DATA_DIR}/media`。Linux 上应提前创建目录并让容器用户可写；不要把本地 `config/config.yaml` 放进镜像。
...
sudo mkdir -p /opt/books-of-time /etc/books-of-time /var/lib/books-of-time/{raw,media}
...
tar -czf books-of-time-files.tar.gz data/raw data/media
```
