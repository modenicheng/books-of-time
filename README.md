...
Alembic revision 是部署契约，必须提交到 Git。修改 ORM 后使用 `uv run alembic revision --autogenerate` 生成 revision，人工审查 upgrade/downgrade，再运行 `uv run alembic upgrade head` 和 `uv run alembic check`。现有无版本数据库的基线登记方法见 [DEPLOYMENT](docs/DEPLOYMENT.md)。
